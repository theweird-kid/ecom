...
func (h *Handler) HandleGetCart(w http.ResponseWriter, r *http.Request) {
	...
	utils.RespondWithJSON(w, 200, models.DatabaseCartRowsToCartRows(cartItems))
...
func (h *Handler) HandleUpdateCart(w http.ResponseWriter, r *http.Request) {
	...
	if prod.Quantity < params.Quantity {
		utils.RespondWithJSON(w, 199, fmt.Sprintf("Only %d units of Product %v available. ", prod.Quantity, prod.ID))
		return
	}
